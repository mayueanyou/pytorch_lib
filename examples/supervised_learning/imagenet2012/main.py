...
dataset_path = "~/datasets/ImageNet2012"
current_path =  os.path.abspath(os.path.dirname(__file__) + os.path.sep + ".")

def main(name):
    tf = Compose([Resize((224,224)),ToTensor()])
    #tf = Compose([transforms.Resize(img_size),ToTensor(),Grayscale()])
    #tf = ToTensor()
    dataset = pl.ImageNet2012(dataset_path,batch_size=128,training_transform=tf,test_transform=tf)
